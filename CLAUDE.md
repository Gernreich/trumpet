...
thing. There is no Python here — the cut files are generated by the sibling repository
**`../octomino-snakes`**, and this repository holds the result.
...
The files here use two stages — `#0000ff` engraves the section number on every part, then
...
- Regenerating from the walk rewrites **every** file in the directory. Ask first.
