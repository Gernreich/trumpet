...
It replaces **`trumpet-curved`**, which built the same instrument *with* elbows. That
repository is no longer checked out here — it lives only at
<https://github.com/Gernreich/trumpet-curved>, and its `CLAUDE.md` is where the shared
house conventions are written down. This file records what is different here, and the
