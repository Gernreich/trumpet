...
**`../octomino-snakes`** and this repository holds the result; the bell and the mouthpiece
have their own generators here, moved across from `../trumpet-curved` because neither is
changed by the elbow question.

It replaces **`../trumpet-curved`**, which built the same instrument *with* elbows. Read
that repository's `CLAUDE.md` for the shared house conventions; this file records only
what is different here, and the difference is the whole point of the design.
...
**The walk is stored in the page**, in `trumpet-curved_no_elbows.html` as a
...
Unchanged from `../trumpet-curved`, and still the error that invalidates a build sheet: a
...
red ramp**, one stage per ring ordered by size, `#000000` on the smallest through `#ff0000`
on the rim. The rings are nested five deep, and cutting smallest-first is inner-before-outer
for every nested pair. `ramp_bell.py` applies it; `verify_bell.py` checks the ramp rises
with ring size. Give it an explicit operation — a per-colour job silently skips any colour
left unmapped.
...
- Regenerating the bore from the walk rewrites **every** net in the root. Ask first.

## Commands

```sh
G=../lasermade-tools
S=../octomino-snakes/generator
```
...
cd $S && python3 bore_split.py \
    ../../trumpet-curved_no_elbows/trumpet-curved_no_elbows.html \
    --write ../../trumpet-curved_no_elbows
```
...
is copied unchanged from `../trumpet-curved`, including the per-sha concurrency group that
repository added after a stuck run blocked its deploy queue. `index.html` is generated and
...
Inherited from `../trumpet-curved` and worth repeating: `doc-audit`'s file check asks
whether a name exists **anywhere** under the repository root. That is right for prose and
