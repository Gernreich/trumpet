...
Those judgements are over a set of ten, not over this page's ${lower(rows.length)} — of
which ${lower(clean.length)} are walls-free. The filters are what carries across; the split
...
the same. Thickness sorts nothing here: all ${lower(solid.length)} of the coils are at
