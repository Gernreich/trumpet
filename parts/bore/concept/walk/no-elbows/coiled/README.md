...
A trumpet bore in 10 × 10mm square section that **coils flat and drops twice**, built from
...
[octagonal torus](https://github.com/Gernreich/trumpet/tree/main/octagonal), which stayed on
the 25 × 25mm channel this bore was first cut at. Cut from 3mm Baltic birch plywood,
millimetre-true at
...
*The whole bore, cut and not yet glued — 50 plates across 8 sections. **This photograph is
of the 25mm build**, which is what was cut before the bore moved to 10mm on 2026-09-05; the
shapes are the same and the plates are the size the old block gave. Click it for the
...
<!-- readme-only -->
**[Read the writeup](https://gernreich.github.io/trumpet/parts/bore/concept/walk/no-elbows/coiled/)** — the same text as
...
— every trumpet, not this one alone; the nine bore nets and the page they
regenerate from are under `bores/coiled/`. GitHub builds it from `main` on every push, so it is never out of date.
...
16mm. Coring it out for the bore to pass through does not shrink it — the four walls stay
and the block still takes up 16mm. **A run of N blocks is 16N mm long.**
...
```
blocksize 16mm   thickness 3mm   burn 0.1   spacing 0.5   inner corners: corner
```
...
The generator is [bore-generator](https://github.com/Gernreich/trumpet/tree/main/tools), which
builds the nets on top of **boxes.py** (Florian Festi, GPL 3.0,
<https://www.festi.info/boxes.py/>).

## Files

**The bore**, in `bore/` — eight nets, one per section:
`bore10-coiled-01of08-bend-DDDR-buttin-cut-files.svg`,
`bore10-coiled-02of08-bend-UUUUL-cut-files.svg`,
`bore10-coiled-03of08-bend-LLLDDDDDDDDDDR-cut-files.svg`,
`bore10-coiled-04of08-bend-RRRD-cut-files.svg`,
`bore10-coiled-05of08-bend-LU-cut-files.svg`,
`bore10-coiled-06of08-bend-UUUUUUL-cut-files.svg`,
`bore10-coiled-07of08-bend-LD-cut-files.svg`,
`bore10-coiled-08of08-bend-LDDDDDDDDDDDD-buttout-cut-files.svg`.
At 25mm the third section needed two sheets and there were nine; at 10mm it fits on one.
...
[trumpet-parts](https://github.com/Gernreich/trumpet/tree/main/parts).

Released under [CC0 1.0](LICENSE).

Bore nets are generated by
[bore-generator](https://github.com/Gernreich/trumpet/tree/main/tools) on top of
**[boxes.py](https://www.festi.info/boxes.py/)** by **Florian Festi** (GPL 3.0),
`burn=0.1`, blocksize 16mm. The bells, the mouthpiece and the text are CC0.
